...
def worker1(key,p,k,capacity,results_avg,recall,f1_score,selected_null,marginList):
    weight_cost = []
...
if __name__ == '__main__':
    ...
    if nm == 1: 
        ...
        for i,(key,p) in enumerate(test_dict.items()):
            pool.apply_async(worker_bi,args=(key,p,i,capacity,results_avg,recall,f1_score,selected_null,marginList))
        pool.close()
        pool.join()
